...
input[type="submit"]:hover{
  ...
  cursor:pointer;
	...
	<?php
	$sql="SELECT * from invites where toname='".$username."'";
	$result=$conn->query($sql);
  if ($result->num_rows == 0) {
    ?>
    <p style="font-size:20px; position:relative; left:550px; top:50px;">You have no Invites..</p>
    <?php
  }
  else {
    ?>
    ...
    <?php
	  while($row=mysqli_fetch_assoc($result)){
	  ?>
	  <tr><td><?php echo $row['fromname']; ?></td><td><input type="submit" value="ACCEPT" onclick="accepting('<?php echo $row['fromname']; ?>')"></td></tr>
	  <?php
	}
  ?>
    </table>
...
?>
<script>
  function accepting(x){
    document.cookie="fromname="+x;
    window.location.href="addconnection.php";
  }
</script>
<script>
